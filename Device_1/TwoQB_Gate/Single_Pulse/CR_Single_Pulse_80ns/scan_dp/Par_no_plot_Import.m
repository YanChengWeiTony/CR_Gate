function [ g, p_g ] = Par_no_plot_Import( input )
% undriven basis
...
omega_basis = 30.8751; %second Qubit's dressed Freq
omegap = omega_basis + dp_CR;
omegaAC = omega_basis + dp_AC;
...
p.sk_basis_num = sk_basis_num;

p.tot_basis_num = pk_basis_num + qk_basis_num + rk_basis_num + sk_basis_num + 2;
...
fid_pz = zeros([1, t_num]);

for aa=1:t_num
    tmp_0 = kron( eye(1+p.tot_basis_num), ZGate(- Z_phase_AC_ratio * AC_phse(aa))) * reshape(sol(aa,:), [(1 + p.tot_basis_num)*lev^2,lev^2]);
%     tmp_U = tmp_0(1:4, 1:4);
%     Buf_U = tmp_U(1:4, 1:4) / (1/(2^2) * trace(tmp_U' * tmp_U))^.5;
%     tmp_00 = tmp_0(1:lev^2, 1:lev^2);
%     tmp_U = tmp_00 / (1/(lev^2) * trace(tmp_00' * tmp_00));
    Buf_U = tmp_0(1:4,1:4);
    fid(aa) = abs(trace(Ideal' * Buf_U))/4;
    myg(aa) = trace(Ideal' * Buf_U);  
    

    tmp_pU = tmp_0(lev^2+1:(1+p.tot_basis_num)*lev^2, 1:lev^2);
    for b=1:p.tot_basis_num
        Buf_pU = tmp_pU(1 + (b-1) * lev^2 : 4 + (b-1) * lev^2 , 1:4);
        fid_p(b, aa) = trace(Ideal' * Buf_pU)/4;
    end
    % Z_phase_AC_ratio derivative
    BufZ = Perf_dr_ZI * tmp_0(1:lev^2, 1:lev^2);
    pzU = -1i * -1 * AC_phse(aa) * BufZ(1:4, 1:4);
    fid_pz(aa) = trace(Ideal' * pzU)/4;
end
...
g = 1 - fid(end)
mygoal = myg(end);
p_g = zeros([p.tot_basis_num+1+not_optimal_parms,1]);

for aa=1:p.tot_basis_num
    p_g(aa) = -real(conj(mygoal) / abs(mygoal) * fid_p(aa,end));
end
% Z_phase_AC_ratio derivative
p_g(p.tot_basis_num + 1) = -real(conj(mygoal) / abs(mygoal) * fid_pz(end));
...
for aa=1:t_num
    tmp = Hm(t(aa),p);
    Buf = tmp(1:4,1:4);
    coefIX(aa) = real(trace(Buf * x2)) / 4;
    coefZX(aa) = real(trace(Buf * zx)) / 4;
    coefIY(aa) = real(trace(Buf * y2)) / 4;
    coefZY(aa) = real(trace(Buf * zy)) / 4;
end
...
figure(12)
plot(t, fid_pz(:), 'linewidth', 1);
xlabel('time(ns)')
ylabel('\partial_\alpha Fidelity')
grid on
...
global D_dr XI_dr YI_dr fx0_ary fy0_ary aI_dr
...
CR_term_X_RWA = fx0_ary(idx) * (aI_dr * exp(-1i*((p.omega_basis-p.omegap)*t - p.phip) )...
    + (aI_dr)' * exp(1i*((p.omega_basis-p.omegap)*t - p.phip)) );

CR_term_Y_RWA = -1 * fy0_ary(idx) * -1i * (aI_dr * exp(-1i*((p.omega_basis-p.omegap)*t - p.phip))...
    - (aI_dr)' * exp(1i*((p.omega_basis-p.omegap)*t - p.phip)) );

Hm = U_ud' * ( CR_term_X_RWA + CR_term_Y_RWA + ...
    Activeterm(t, p) ) * U_ud;
...
global XI_dr YI_dr IX_dr IY_dr D_dr
global aI_dr Ia_dr fx0_ary fy0_ary fACx_ary fACy_ary
...
Buf = heaviside(t - p.ts) -  heaviside(t - p.tf);

% dHm/df (fx)
dHm_dfx =  U_ud' * (aI_dr * exp(-1i*((p.omega_basis-p.omegap)*t - p.phip) )...
    + (aI_dr)' * exp(1i*((p.omega_basis-p.omegap)*t - p.phip))) * U_ud;


% df/dpk (fx)
k = (1:p.pk_basis_num)';
dfx_dpk = sin(pi * k /p.t_len * (t -  p.ts)) * Buf;
...
dHm_dfy = U_ud' * 1i * (aI_dr * exp(-1i*((p.omega_basis-p.omegap)*t - p.phip))...
    - (aI_dr)' * exp(1i*((p.omega_basis-p.omegap)*t - p.phip))) * U_ud;


% df/dqk (fy)
k = (1:p.qk_basis_num)';
dfy_dqk = sin(pi * k /p.t_len * (t - p.ts)) * Buf;

out_fy = kron(dfy_dqk, dHm_dfy);



% dHm/df (fACx)
dHm_dfACx =  U_ud' * (Ia_dr * exp(-1i*((p.omega_basis-p.omegaAC)*t - p.phac) )...
    + (Ia_dr)' * exp(1i*((p.omega_basis-p.omegaAC)*t - p.phac))) * U_ud;

% df/drk (fACx)
k = (1:p.rk_basis_num)';
dfACx_drk = sin(pi * k /p.t_len * (t - p.ts)) * Buf;

out_fACx = kron(dfACx_drk, dHm_dfACx);



% dHm/df (fACy)
dHm_dfACy = U_ud' * 1i * (Ia_dr * exp(-1i*((p.omega_basis-p.omegaAC)*t - p.phac))...
    - (Ia_dr)' * exp(1i*((p.omega_basis-p.omegaAC)*t- p.phac) )) * U_ud;


% df/drk (fACy)
k = (1:p.sk_basis_num)';
dfACy_dsk = sin(pi * k /p.t_len * (t - p.ts)) * Buf;

out_fACy = kron(dfACy_dsk, dHm_dfACy);

% dH/d(dp_CR) (RWA)
dH_ddp_CR_X_RWA = fx0_ary(idx) * (aI_dr * (1i * t) * exp(-1i*((p.omega_basis-p.omegap)*t - p.phip) )...
    + (aI_dr)' * (-1i * t) * exp(1i*((p.omega_basis-p.omegap)*t - p.phip)));

dH_ddp_CR_Y_RWA = -1 * fy0_ary(idx) * -1i * (aI_dr * (1i * t) * exp(-1i*((p.omega_basis-p.omegap)*t - p.phip))...
    - (aI_dr)' * (-1i * t) * exp(1i*((p.omega_basis-p.omegap)*t - p.phip)));

dH_ddp_CR = dH_ddp_CR_X_RWA + dH_ddp_CR_Y_RWA;

% dH/d(dp_AC) (RWA)

dH_ddp_AC_X_RWA = fACx_ary(idx) * (Ia_dr * (1i * t) * exp(-1i*((p.omega_basis-p.omegaAC)*t - p.phac) )...
    + (Ia_dr)' *(-1i * t)* exp(1i*((p.omega_basis-p.omegaAC)*t - p.phac)));

dH_ddp_AC_Y_RWA = -1 * fACy_ary(idx) * -1i * (Ia_dr * (1i * t) * exp(-1i*((p.omega_basis-p.omegaAC)*t - p.phac))...
    - (Ia_dr)' * (-1i * t) * exp(1i*((p.omega_basis-p.omegaAC)*t- p.phac) ));

dH_ddp_AC = dH_ddp_AC_X_RWA + dH_ddp_AC_Y_RWA;

% out
out = [out_fx;out_fy;out_fACx;out_fACy; dH_ddp_CR; dH_ddp_AC];
...
AC_x_RWA = fACx_ary(idx) * (Ia_dr * exp(-1i*((p.omega_basis-p.omegaAC)*t - p.phac) )...
    + (Ia_dr)' * exp(1i*((p.omega_basis-p.omegaAC)*t - p.phac)));

AC_y_RWA = -1 * fACy_ary(idx) * -1i * (Ia_dr * exp(-1i*((p.omega_basis-p.omegaAC)*t - p.phac))...
    - (Ia_dr)' * exp(1i*((p.omega_basis-p.omegaAC)*t- p.phac) ));

% out = (fACx_ary(idx) * cos(p.phac) - fACy_ary(idx) * sin(p.phac)) * IX_dr ...
%     - (fACx_ary(idx) * sin(p.phac) + fACy_ary(idx) * cos(p.phac)) * IY_dr;

out = AC_x_RWA + AC_y_RWA;
