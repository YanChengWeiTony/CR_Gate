function [ output_args ] = Par_CR_Sgle_Pulse( input_args )
% undriven basis
...
% basis num
pk_basis_num = size(pn,2);
qk_basis_num = size(qn,2);
rk_basis_num = size(rn,2);
sk_basis_num = size(sn,2);
%% Parms
...
p.tot_basis_num = pk_basis_num + qk_basis_num + rk_basis_num + sk_basis_num;
...
% dHm/df (fACx)
dHm_dfACx =  U_ud' * ( cos(p.phac) * IX_dr - sin(p.phac) * IY_dr) * U_ud;
...
% dHm/df (fACy)
dHm_dfACy = U_ud' * ( -sin(p.phac) * IX_dr  -  cos(p.phac) *  IY_dr) * U_ud;
...
% out
out = [out_fx;out_fy;out_fACx;out_fACy];
