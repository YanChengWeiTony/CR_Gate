function [ g ] = CR_Sgle_Pulse_Import_test( input )
% undriven basis
...
tf = -0;
% transfer to ary
fx0_ary = myf0_heavside(pn, ts, tf, t_num, t);
...
% omega_ary
ary = 1:lev;
omega_diag1 = diag((ary - 1) * (omega1 - omega_basis) - (ary - 1) .* (ary - 2)/2 * alpha1);
omega_diag2 = diag((ary - 1) * (omega2 - omega_basis) - (ary - 1) .* (ary - 2)/2 * alpha2);
...
% AC_shift = -.5*(((2 * fx0_ary).^2 + (2 * fy0_ary).^2 + detuneDress12^2).^.5 - detuneDress12)/2;
Detune_1p
AC_shift_2 = -1*(((2 * fx0_ary).^2 + (2 * fy0_ary).^2 + (alpha1 - Detune_1p)^2).^.5 - (alpha1 - Detune_1p))/2;
AC_shift_1 = -1*(((2 * fx0_ary).^2 + (2 * fy0_ary).^2 + Detune_1p^2).^.5 - Detune_1p)/2;

AC_phse = cumsum(AC_shift_1 + AC_shift_2) * dt;

for aa=1:t_num
    tmp_0 = ZGate(-Z_phase_AC) * reshape(sol(aa,:), [(1)*lev^2,lev^2]);
%     tmp_0 = ZGate(-AC_phse(aa)) * reshape(sol(aa,:), [(1)*lev^2,lev^2]);
    Buf_U = tmp_0(1:4,1:4);
...
function out = myf0_heavside(pn, ts, tf, t_num, t)
t_len = tf - ts;
...
CR_term_X_RWA_12 = fx0_ary(idx) * (aI_dr * exp(-1i*((p.omega_basis-p.omegap)*t - p.phip) )...
    + (aI_dr)' * exp(1i*((p.omega_basis-p.omegap)*t - p.phip)));

CR_term_Y_RWA_12 = -1 * fy0_ary(idx) * -1i * (aI_dr * exp(-1i*((p.omega_basis-p.omegap)*t - p.phip))...
    - (aI_dr)' * exp(1i*((p.omega_basis-p.omegap)*t - p.phip)));


Hm = U_ud' * (CR_term_X_RWA_12 + CR_term_Y_RWA_12 + ...
    Activeterm12(t, p) ) * U_ud;
