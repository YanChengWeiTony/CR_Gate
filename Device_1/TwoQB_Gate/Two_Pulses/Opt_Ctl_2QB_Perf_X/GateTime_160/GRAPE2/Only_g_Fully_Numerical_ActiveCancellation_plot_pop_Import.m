function  [g] = Only_g_Fully_Numerical_ActiveCancellation_plot_pop_Import(myx)
%% Spec
...
for a=1:t_num
    ...
    All_pop(:, a) = reshape(abs(tmp_0(1:4, 1:end)).^2, [4*lev^2,1]);
end
...
poptable_Q1 = [0 0 1 1 0 2 1 2 2 0 3 1 3 2 3 3];
poptable_Q2 = [0 1 0 1 2 0 2 1 2 3 0 3 1 3 2 3];

figure(6)
subplot(4,1,1)
for idx=5:16
    name = strcat(num2str(poptable_Q1(idx)), num2str(poptable_Q2(idx)));
    plot(t, All_pop(4 * (idx-1) + 1, :), 'Linewidth', 1, 'DisplayName',strcat('00 to ', name));hold on
    pName{idx-4} = name;
end
legend(pName);
...
for idx=5:16
    name = strcat(num2str(poptable_Q1(idx)), num2str(poptable_Q2(idx)));
    plot(t, All_pop(4 * (idx-1) + 2, :), 'Linewidth', 1, 'DisplayName',strcat('01 to ', name));hold on
    pName{idx-4} = name;
end
legend(pName);
...
for idx=5:16
    name = strcat(num2str(poptable_Q1(idx)), num2str(poptable_Q2(idx)));
    plot(t, All_pop(4 * (idx-1) + 3, :), 'Linewidth', 1, 'DisplayName',strcat('10 to ', name));hold on
    pName{idx-4} = name;
end
legend(pName);
...
for idx=5:16
    name = strcat(num2str(poptable_Q1(idx)), num2str(poptable_Q2(idx)));
    plot(t, All_pop(4 * (idx-1) + 4, :), 'Linewidth', 1, 'DisplayName',strcat('11 to ', name));hold on
    pName{idx-4} = name;
end
legend(pName);
...
ylabel('Amp(GHz)')
grid on
legend('fCR-x', 'fCR-y', 'pi-gate', 'fAC-x', 'fCR-y')
...
% 4-lev
gf = zeros([lev^2,1]);gf(4) = 1;

fg = zeros([lev^2,1]);fg(3 * lev + 1) = 1;

ef = zeros([lev^2,1]);ef(lev + 4) = 1;

fe = zeros([lev^2,1]);fe(3 * lev + 2) = 1;

hf = zeros([lev^2,1]);hf(2 * lev + 4) = 1;

fh = zeros([lev^2,1]);fh(3 * lev + 3) = 1;

ff = zeros([lev^2,1]);ff(3 * lev + 4) = 1;
...
[M10, I10] = max(abs(gf' * V));
sgn10 = sign(V(4,I10));
S(1:lev^2, 10) = V(1:lev^2, I10)*sgn10;
D(10,10) = F(I10, I10);
V(:, I10) = [];
F(:,I10) = [];
F(I10, :) = [];

[M11, I11] = max(abs(fg' * V));
sgn11 = sign(V(3*lev + 1,I11));
S(1:lev^2, 11) = V(1:lev^2, I11)*sgn11;
D(11,11) = F(I11, I11);
V(:, I11) = [];
F(:,I11) = [];
F(I11, :) = [];

[M12, I12] = max(abs(ef' * V));
sgn12 = sign(V(lev + 4,I12));
S(1:lev^2, 12) = V(1:lev^2, I12)*sgn12;
D(12,12) = F(I12, I12);
V(:, I12) = [];
F(:,I12) = [];
F(I12, :) = [];

[M13, I13] = max(abs(fe' * V));
sgn13 = sign(V(3 * lev + 2,I13));
S(1:lev^2, 13) = V(1:lev^2, I13)*sgn13;
D(13,13) = F(I13, I13);
V(:, I13) = [];
F(:,I13) = [];
F(I13, :) = [];

[M14, I14] = max(abs(hf' * V));
sgn14 = sign(V(2 * lev + 4,I14));
S(1:lev^2, 14) = V(1:lev^2, I14)*sgn14;
D(14,14) = F(I14, I14);
V(:, I14) = [];
F(:,I14) = [];
F(I14, :) = [];

[M15, I15] = max(abs(fh' * V));
sgn15 = sign(V(3 * lev + 3,I15));
S(1:lev^2, 15) = V(1:lev^2, I15)*sgn15;
D(15,15) = F(I15, I15);
V(:, I15) = [];
F(:,I15) = [];
F(I15, :) = [];

[M16, I16] = max(abs(ff' * V));
sgn16 = sign(V(3 * lev + 4,I16));
S(1:lev^2, 16) = V(1:lev^2, I16)*sgn16;
D(16,16) = F(I16, I16);
V(:, I16) = [];
F(:,I16) = [];
F(I16, :) = [];
