function  [g, p_g] = par_XI_plot_Import(myx)
%% Spec

% {an, bn, cn, dn, en, dp_offset, dp_offset2}
% {an, cn, dn}: sin
% {bn, en}: cos
%% Fixed input
...
en = input(ak_basis_num+bk_basis_num+ck_basis_num+dk_basis_num+1:end-2);
...
% 4 x 4 matrix
global x y z zx zy x1 x2 y2 zz z1 a a1 a2
x = [0, 1;1 0];
y = [0, -1j; 1j, 0];
z = [1, 0; 0, -1];
a = (x + 1i * y) / 2;
...
% two qubits
global II XI YI IX IY XI_dr YI_dr aI_dr Ia_dr IX_dr IY_dr D_dr Perf_dr_XI Perf_dr_aI Perf_dr_Ia Perf_dr_IX Perf_dr_IY
global Nq1 Num1_dr Num2_dr
...
Perf_dr_Ia = zeros(lev^2);
Perf_dr_Ia(1:4, 1:4) = a2;
...
p.tot_basis_num = ak_basis_num + bk_basis_num + ck_basis_num + dk_basis_num + ek_basis_num + 2;
...
plot(t, Pi_fx20_ary /(2*pi), '-', 'linewidth', 1,'Color', [0 .6 1]); hold on
plot(t, Pi_fy20_ary /(2*pi), '-', 'linewidth', 1, 'Color', [1 0 0]); hold on
plot(t, ones(size(t)) * (dp_offset2) /(2*pi), '-', 'linewidth', 1, 'Color', [0 1 1]); hold on
xlabel('time(ns)')
ylabel('Amp/(2\pi)(GHz)')
grid on
legend('fx', 'fy', 'detune', 'fx2', 'fy2', 'detune2')
...
Buf = heaviside(t - ts1) -  heaviside(t - tf1);

out = zeros([1, t_num]);

for idx=1:basis_num
    out = out + an(idx) * (sin(idx * pi / tgPi * (t - ts1))) .* Buf;
end
...
for idx=1:basis_num
    out = out + an(idx) * (cos(idx * pi / tgPi * (t - ts1))) .* Buf;
end
...
global aI_dr Num1_dr Ia_dr Num2_dr

ts1 = p.t_CR -2 *p.tgPi - 1 * p.t_len;
tf1 =  p.t_CR-1 *p.tgPi - 1 * p.t_len;
...
dfx_dan = (sin(idx_a * pi / p.tgPi * (t - ts1))) .* Buf;
dfy_dbn = (cos(idx_b * pi / p.tgPi * (t - ts1))) .* Buf;
ddp_dcn = (sin(idx_c * pi / p.tgPi * (t - ts1))) .* Buf;
dfx_ddn = (sin(idx_d * pi / p.tgPi * (t - ts1))) .* Buf;
dfy_den = (cos(idx_e * pi / p.tgPi * (t - ts1))) .* Buf;
...
par_dn = 0.4580 * kron(dfx_ddn, par_fx2);
par_en = 0.4580 * kron(dfy_den, par_fy2);

par_offset = -5.62*Num1_dr;
par_offset2 = -5.62*Num2_dr;
% out
out = [par_an;par_bn;par_cn;par_dn;par_en;par_offset;par_offset2];
