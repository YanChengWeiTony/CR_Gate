function  [g, p_g] = pg_g_AC_plot_Import(myx)
%% Spec
...
p.omegap = omegap;
p.omega1 = (D_dr(3,3)+D_dr(4,4))/2 - (D_dr(1,1) + D_dr(2,2))/2 + omegap; %lab frame
p.alpha = -1 * ((D_dr(6,6) + D_dr(1,1) - 2*D_dr(3,3))/2 + (D_dr(8,8) +  D_dr(2,2) - 2* D_dr(4,4))/2); % lab frame
p.t_CR = t_CR;
p.t_len = t_len;
...
global XI_dr YI_dr IX_dr IY_dr
...
% dHm/df (fx)
dHm_dfx =  cos(p.phip) * XI_dr - sin(p.phip) * YI_dr + p.B * (cos(p.phac) * IX_dr - sin(p.phip) * IY_dr );
...
% dHm/df (fy)
dHm_dfy =  -sin(p.phip) * XI_dr -  cos(p.phip) * YI_dr - p.B * (sin(p.phac) * IX_dr + cos(p.phip) * IY_dr );
