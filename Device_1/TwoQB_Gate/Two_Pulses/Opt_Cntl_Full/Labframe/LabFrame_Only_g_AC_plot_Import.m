function  [g] = LabFrame_Only_g_AC_plot_Import(myx)
%% Spec
...
% omega_ary
ary = 1:lev;
omega_diag1 = diag((ary - 1) * (omega1 - 0) - (ary - 1) .* (ary - 2)/2 * alpha1);
omega_diag2 = diag((ary - 1) * (omega2 - 0) - (ary - 1) .* (ary - 2)/2 * alpha2);
...
p.omegap = omegap;
p.omega1 = (D_dr(3,3)+D_dr(4,4))/2 - (D_dr(1,1) + D_dr(2,2))/2;
...
Hm = D_dr + ...
    2*(fx0_ary(idx) * cos(p.omegap * (t-p.t_measure) + p.phip) - fy0_ary(idx) * sin(p.omegap * (t-p.t_measure) +  p.phip)) * XI_dr + ...
    1*Activeterm(t, p) + ...
    xtermpi(t, p);
...
x_term = fx * 2*cos(p.omega1 * t + ph) * (aI_dr + (aI_dr)');
y_term = fy * 2*sin(p.omega1 * t + ph) * (aI_dr + (aI_dr)');
...
% term
out = p.B * 2 * (fx0_ary(idx) * cos(p.omegap * (t-p.t_measure) + p.phac) - fy0_ary(idx) * sin(p.omegap * (t-p.t_measure) + p.phac)) * IX_dr;
end
